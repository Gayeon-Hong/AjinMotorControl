...
function AxcTriggerWriteAbsRamData(lChannelNo : LongInt; dwAddr : DWord; dwData : DWord) : DWord; stdcall;
	...
	// dwDirection	--> 0x0(default) : 사용되지 않는 변수로, 입력하지 않아도 된다.
	//----------------------------------------------------------------------------------------------------------------------------------
function AxcTriggerSetAbs(lChannelNo : LongInt; dwTrigNum : DWord; dwTrigPos : PDWord; dwDirection : DWord = 0) : DWord; stdcall;
	...
	// dwDirection	--> 0x0(default) : 사용되지 않는 변수로, 입력하지 않아도 된다.
	//----------------------------------------------------------------------------------------------------------------------------------
function AxcTriggerSetAbsDouble(lChannelNo : LongInt; dwTrigNum : DWord; dTrigPos : PDouble; dwDirection : DWord = 0) : DWord; stdcall;
