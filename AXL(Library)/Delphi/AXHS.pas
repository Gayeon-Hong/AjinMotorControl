...
const
    ...
    AXT_PCIE_Rxx05ECAT                  = $D8;          // PCIe bus, Half size, ECAT(DSP Based) controller.
    AXT_PCI_Rxx05MLIII                  = $D9;          // PCI bus, Half size, Mechatrolink III 32 axis(DSP Based) controller.
    AXT_PCI_Rxx05SIIIH                  = $DA;          // PCI bus, Half size, Sscnet III / H  32 axis(DSP Based) controller.
    AXT_PCI_Rxx05RTEX                   = $DB;          // PCI bus, Half size, RTEX 32 axis(DSP Based) controller.
    AXT_PCI_Rxx05ECAT                   = $DC;          // PCI bus, Half size, ECAT(DSP Based) controller.
    ...
    AXT_SIO_RMEMORY_MLIII               = $CC;          // Memory Access type module, MechatroLink III
    AXT_SIO_RAVCI8MLII			        = $CE;		// DC VOLTAGE/CURRENT INPUT MODULE, 8 points (Product by M-SYSTEM), For MLII only
    ...
    { 모션제품 보간 사용축 정의         }
    INTERPOLATION_AXIS2                 = $2;           // 2축을 보간으로 사용할 때
    INTERPOLATION_AXIS3                 = $3;           // 3축을 보간으로 사용할 때
    INTERPOLATION_AXIS4                 = $4;           // 4축을 보간으로 사용할 때
    { 모션제품 연속보간 방법 정의       }
    ...
    QIDRIVE_STATUS_20                   = $0100000;     // Bit 20, 구동 예약 Queue 가득 ?                                    
    ...
    QIINTBANK1_21                       = $00200000;    // Bit 21, 구동 예약 설정 Queue 가득?                              
    QIINTBANK1_22                       = $00400000;    // Bit 22, 트리거 발생거리 주기/절대위치 Queue 비워짐.              
    QIINTBANK1_23                       = $00800000;    // Bit 23, 트리거 발생거리 주기/절대위치 Queue 가득?               
    ...
    QIINTBANK2_0                        = $00000001;    // Bit 0,  스크립트 #1 읽기 명령 결과 Queue 가 가득?.                                  
    QIINTBANK2_1                        = $00000002;    // Bit 1,  스크립트 #2 읽기 명령 결과 Queue 가 가득?.                                  
