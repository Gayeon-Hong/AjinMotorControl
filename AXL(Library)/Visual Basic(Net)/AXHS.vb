...
Module AXHS
    ...
    Public Const AXT_PCIE_Rxx05ECAT As Short = &HD8S                        ' PCIe bus, Half size, ECAT(DSP Based) controller.
    Public Const AXT_PCI_Rxx05MLIII As Short = &HD9S                        ' PCI bus, Half size, Mechatrolink III 32 axis(DSP Based) controller.
    Public Const AXT_PCI_Rxx05SIIIH As Short = &HDAS                        ' PCI bus, Half size, Sscnet III / H  32 axis(DSP Based) controller.
    Public Const AXT_PCI_Rxx05RTEX As Short = &HDBS                         ' PCI bus, Half size, RTEX 32 axis(DSP Based) controller.
    Public Const AXT_PCI_Rxx05ECAT As Short = &HDCS                         ' PCI bus, Half size, ECAT(DSP Based) controller.
    ...
    Public Const AXT_SIO_RAVI4MLII = &HCBS                                  ' DC VOLTAGE/CURRENT INPUT MODULE, 4 points (Product by M-SYSTEM), For MLII only
    Public Const AXT_SIO_RMEMORY_MLIII = &HCCS                              ' Memory Access type module, MechatroLink III
    ...
    Public Const AXT_RT_CNT_NOT_TRIGGER_ENABLE As Short = 3253              ' CNT Trigger 출력 기능이 활성화되어 있지 않음
    ...
    Public Const AXT_RT_CNT_CMD_EXE_TIMEOUT As Short = 3308                 ' CNT 모듈 데이터입력 시간초과 했을때
    Public Const AXT_RT_CNT_INVALID_VELOCITY As Short = 3309                '유효하지않는 CNT 속도
    Public Const AXT_RT_PROTECTED_DURING_PWMENABLE As Short = 3310          ' PWM Enable 되어 있는 상태에서 사용 못 함
    Public Const AXT_RT_CNT_INVALID_TABLEPOS As Short = 3311                ' 유효하지 않은 CNT TABLE 번호 
    Public Const AXT_RT_CNT_DIMENSION_ERROR As Short = 3312                 ' 해당 Dimension 설정 상태에서는 사용할 수 없음
	...
	Public Const AXT_ERROR_HELICAL_ZPOS_DISTANCE_ZERO As Short = 4173       ' 헬리컬 맵핑된 Z축의 이동량이 0일 때
    ...
    Public Const AXT_RT_ERROR_RING_COUNTER_ENABLE As Short = 4420           ' Ring Counter 기능이 사용 중
    Public Const AXT_RT_ERROR_RING_COUNTER_OUT_OF_RANGE As Short = 4421     ' Ring Counter 사용 중 범위 밖 명령 위치 호출(POS_ABS_LONG_MODE or POS_ABS_SHORT_MODE 일 경우)
    Public Const AXT_RT_ERROR_SOFT_LIMIT_ENABLE As Short = 4430             ' Software Limit 기능이 사용 중
    Public Const AXT_RT_ERROR_SOFT_LIMIT_NEGATIVE As Short = 4431           ' 이동할 위치가 Negative Software Limit을 벗어남
    Public Const AXT_RT_ERROR_SOFT_LIMIT_POSITIVE As Short = 4432           ' 이동할 위치가 Positive Software Limit을 벗어남
    ...
    Public Const INTERPOLATION_AXIS2 As Short = &H2S                        ' 2축을 보간으로 사용할 때
    Public Const INTERPOLATION_AXIS3 As Short = &H3S                        ' 3축을 보간으로 사용할 때
    Public Const INTERPOLATION_AXIS4 As Short = &H4S                        ' 4축을 보간으로 사용할 때
    ...
    Public Const QIDRIVE_STATUS_20 As Integer = &H100000                    ' Bit 20, 구동 예약 Queue 가득 ?
    ...
    Public Const QIINTBANK1_21 As Integer = &H200000                        ' Bit 21, 구동 예약 설정 Queue 가득?
    Public Const QIINTBANK1_22 As Integer = &H400000                        ' Bit 22, 트리거 발생거리 주기/절대위치 Queue 비워짐.
    Public Const QIINTBANK1_23 As Integer = &H800000                        ' Bit 23, 트리거 발생거리 주기/절대위치 Queue 가득?
    ...
    Public Const QIINTBANK2_0 As Short = &H1S                               ' Bit 0,  스크립트 #1 읽기 명령 결과 Queue 가 가득?.
    Public Const QIINTBANK2_1 As Short = &H2S                               ' Bit 1,  스크립트 #2 읽기 명령 결과 Queue 가 가득?.
